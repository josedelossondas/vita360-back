...
def add_evidence(ticket_id: int, image_url: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):

    evidence = Evidence(ticket_id=ticket_id, image_url=image_url)
    db.add(evidence)
    ...
    return {"message": "Evidence added and ticket resolved"}