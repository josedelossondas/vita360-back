...
async def fleet_ws(websocket: WebSocket):
    ...
    await websocket.send_text(json.dumps(sim.get_current_state()))
    try:
        while True:
...
def _openai_chat(messages, max_tokens=60):
...
def calculate_priority_factors_with_ai(title, description):
    if not _openai_available():
    ...
    try:
        raw = _openai_chat([
            {
                "role": "system",
                "content": "Return ONLY valid JSON with numeric fields (0-100)."
            },
            {
                "role": "user",
                "content": f"""
        ...
        data = json.loads(raw)
        return {k: int(data.get(k, 50)) for k in DEFAULT_PRIORITY_FACTORS.keys()}
    except:
        return DEFAULT_PRIORITY_FACTORS.copy()

def compute_priority_score_from_factors(factors, weights):
    total = sum(float(factors.get(k, 0)) * float(w) for k, w in weights.items())
...
>>>>>>> 4ce4ef61c7ea2e33a4056564666f91368bb74170
